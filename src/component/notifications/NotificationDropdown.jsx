...
const NotificationDropdown = () => {
    ...
    return (
        <div
            className={`absolute right-0 mt-2 w-80 rounded-md shadow-lg ${darkMode ? "bg-gray-800" : "bg-white"} ring-1 ring-black ring-opacity-5 z-50 max-h-96 overflow-y-auto`}
        >
            <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                <h3 className="text-lg font-semibold">Notifications</h3>
                {notifications.some((notification) => !notification.read) && (
                    <button onClick={markAllAsRead} className="text-xs text-rose-600 hover:text-rose-700 transition-colors">
                        Mark all as read
                    </button>
                )}
            </div>
