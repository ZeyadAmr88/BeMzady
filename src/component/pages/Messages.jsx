  ...
  const confirmDelete = async () => {
    try {
      ...
      setConversations((prev) =>
        Array.isArray(prev)
          ? prev.filter((conv) => conv && conv._id !== deleteConfirmationId)
          : []
      );

      if (
        selectedConversation &&
        selectedConversation._id === deleteConfirmationId
      ) {
          ...
          {filteredConversations.length === 0 ? (
            ...
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredConversations.map((conversation) => {
                ...
                // Check if the last message is unread and current user is the recipient
                const isUnread =
                  lastMessage &&
                  !lastMessage.isRead &&
