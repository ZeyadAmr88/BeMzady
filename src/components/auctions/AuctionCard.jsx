...
const AuctionCard = ({ auction, isHot = false }) => {
    ...
    const getCurrentPrice = () => {
        if (auction.bids && auction.bids.length > 0) {
            return auction.currentPrice || auction.bids[auction.bids.length - 1].amount;
        }
    ...
    return (
        <div className="bg-surface rounded-lg overflow-hidden shadow-lg transition-transform duration-300 hover:transform hover:scale-105 relative">
            ...
            <div className="relative h-48 overflow-hidden">
                <img
                    src={auction.auctionCover || (auction.auctionImages && auction.auctionImages.length > 0
            ...
            {/* Auction details */}
            <div className="p-4">
                <h3 className="text-lg font-bold mb-2 text-text truncate">{auction.title}</h3>
                <p className="text-text-muted text-sm mb-3 line-clamp-2">{auction.description}</p>

                <div className="flex justify-between items-center mb-3">
                    <div>
                        <span className="text-text-muted text-xs">Current Price</span>
                        <p className="text-primary font-bold text-lg">{formatCurrency(getCurrentPrice())}</p>
                    </div>
                    {auction.buyNowPrice && (
                        <div>
                            <span className="text-text-muted text-xs">Buy Now</span>
                            <p className="text-secondary font-bold">{formatCurrency(auction.buyNowPrice)}</p>
                        </div>
                    )}
                </div>
